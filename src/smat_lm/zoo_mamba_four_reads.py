...
class MqarMambaFourReads(SmatMamba2MR):
    def __init__(self, d_model, layer_idx=0, d=3, prebuild_lengths=(64, 128, 256), d_state=16, headdim=16, memory_tied_features=False, **kwargs):
        super().__init__(d_model, layer_idx=layer_idx, d=d, d_state=d_state,
            headdim=headdim, reset=True, read='chash', hash_codim=1,
        ...
        if d >= 2:
            for length in prebuild_lengths:
                ...
                for ca in self.ca[str(length)].values():
                    ...
                    ca.hard_k1 = True
            self.specs.clear()
        self.memory_tied_features = bool(memory_tied_features)
        if self.memory_tied_features and d >= 2:
            assert self.kernel == 'id' and self.hash_src == 'hidden' and self.hash_conv
            m = self.mixer
            assert m.ngroups == 1
            # Reuse the query projection initialization; preserve all existing parameters.
            self.memory_qk = torch.nn.Linear(d_model, m.d_state, bias=False)
            start = 2*m.d_ssm + m.ngroups*m.d_state
            with torch.no_grad():
                self.memory_qk.weight.copy_(m.in_proj.weight[start:start+m.d_state])
