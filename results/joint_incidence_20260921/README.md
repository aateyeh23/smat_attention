...
*In this repository:* only the geometric-SMat arm's run records are kept
(`geometry/`, seeds 123, 456, 789): they are the GDN + SMat (d = 3) row of
Table `mom_macro`, final-test macro accuracy 58.92 (4.22).  The random-incidence
and bucket arms appear in `report.md` and `summary.json` but their run folders,
the per-example arrays, shard status and logs are not kept.  Every module the
frozen trainer imports (`validated.json`, `imports`) is byte-identical to its copy
in `results/paper_seeds_20260920/source/joint/code/` or `third_party/zoology/`;
the launcher is `results/joint_mom_20260921/source/incidence_ablation/`.
...
`python code/incidence_ablation/collect.py`.
