...
cd /u/an author/smat_attention/cluster
...
export MQAR_RUN_DIR="$PWD/results/mqar_gdn_interleaved/w16w32_s123"
export TRITON_CACHE_DIR="$PWD/results/mqar_gdn_reset/check_cache"
mkdir -p "$MQAR_RUN_DIR"
cp zoo_mqar_interleave.py zoo_gdn_interleaved_configs.py zoo_gdn_reset_configs.py zoo_smat_gdn.py zoo_mqar_resume.py run_mqar_interleaved.sh "$MQAR_RUN_DIR/"
pids=()
for width in ${MQAR_INTERLEAVE_WIDTHS:-16 32}; do
    env ZOO_DM="$width" python -u -m zoology.launch zoo_gdn_interleaved_configs.py >> "$MQAR_RUN_DIR/w${width}-d3.log" 2>&1 &
    pids+=("$!")
done
status=0
