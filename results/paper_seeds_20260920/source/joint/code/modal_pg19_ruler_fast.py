...
app = modal.App('pg19-ruler-verified-decode')
...
if modal.is_local():
    ...
    for name in ('eval_pg19_ruler_fast.py','ruler_greedy_decode.py'):
...
def evaluate(arm: str='mamba2-smat-d3', verify_only: bool=True, batch: int=4):
    ...
    output = base/('verification' if verify_only else 'fast')/arm
    ...
    command = [sys.executable,'-P','-u','/opt/pg19-rank64/lm/eval_pg19_ruler_fast.py',
        '--checkpoint',str(checkpoint),'--data','/ruler-data','--output',str(output),
        '--reference',str(base/'full'/arm),'--batch',str(batch)]
    if verify_only:
    ...
    try:
        ...
        phase('complete')
        return json.loads((output/('verification.json' if verify_only else 'result.json')).read_text())
...
def sweep(verify_only: bool=True):
    calls = {arm:evaluate.spawn(arm,verify_only) for arm in ARMS}
