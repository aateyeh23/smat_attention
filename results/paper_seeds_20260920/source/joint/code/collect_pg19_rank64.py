"""Collect lightweight metrics for active d=2/3 rank64 PG19 arms."""
import argparse
...
def collect(variant="", dimensions=(2,3)):
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        rows=list(pool.map(fetch,[f'{f}-d{d}' for f in ('gdn','mamba2') for d in dimensions]))
...
if __name__=='__main__':
    ...
    p.add_argument('--variant',default='fusedwrite-b2-ac0')
    p.add_argument('--dimensions',type=int,nargs='+',choices=(2,3,4),default=[2,3])
    args=p.parse_args()
    while True:
        done=collect(args.variant,args.dimensions)
