...
import os
import sys
sys.path[:0]=['/opt/transport','/opt/smat/the GPU cluster','/opt/smat/smat','/opt/zoology','/opt/fla-source']
...
for config in configs:
    config.model.sequence_mixer.kwargs.update(manifest['kwargs'])
ck=torch.load(root/'w32-d3.pt',map_location='cpu',weights_only=False)
    ...
    for mods in m.ca.values():
        for ca in mods.values():
            ...
            def forward(*args,ca=ca,forward_original=forward_original,**kwargs):
                ...
                if context['mode']=='all_planes_mean':
                    n=context['x'].shape[1]//2
                    full_memory=torch.einsum('blr,blp->brp',args[2][:,:n],args[3][:,:n])
                    return torch.einsum('blr,brp->blp',args[1][:,n:],full_memory)/ca.n_cosets
                if context['mode']=='normal':
                    ...
                    # Does the shared writer projection give the query the
                    # corresponding value's address? This is diagnosis only.
                    normalized_W=ca.W/ca.W.norm(dim=-1,keepdim=True).clamp_min(1e-6)
                    z=torch.einsum('blm,hkm->bhlk',ca.ln(args[0][:,n:]),normalized_W)
                    z=z*ca.gamma[None,:,None]+ca.b[None,:,None]
                    coordinates=(torch.special.ndtr(z)*ca.q).clamp(0.,ca.q-1e-4).floor().long()
                    place=coordinates.new_tensor([ca.q**(ca.dim-1-j) for j in range(ca.dim)])
                    query_cell=(coordinates*place).sum(-1)
                    add(key,'query_hash_matches_target',(query_cell==target)[hm])
                    query_incident=ca.M.flatten(0,1).bool()[reads,query_cell[...,None]]
                    add(key,'reads_through_query_cell',query_incident[hm])
                    add(key,'all_reads_through_query_cell',query_incident.all(-1)[hm])
                    ...
                    target_score=scores.gather(-1,expected[...,None]).squeeze(-1).abs()
                    add(key,'target_absolute_content_share',(target_score/scores.abs().sum(-1).clamp_min(1e-12))[hm])
                    target_routed=routed.gather(-1,expected[...,None]).squeeze(-1).abs()
                    add(key,'target_absolute_routed_share',(target_routed/routed.abs().sum(-1).clamp_min(1e-12))[hm])
                    ...
                    if m.memory_incidence_rescale:
                        contribution=contribution*(ca.n_cosets if ca.mode=='plane' else ca.N0)
...
with torch.no_grad():
    for mode in (('normal',) if os.environ.get('PROBE_ONLY_NORMAL')=='1' else
                 ('normal','memory_off','no_decay','no_delta_transport','no_transport','all_planes_mean','oracle_reads')):
        context['mode']=mode
...
(root/'checkpoint-probe.json').write_text(json.dumps(results,indent=2)+'\n')
