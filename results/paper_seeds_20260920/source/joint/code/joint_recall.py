...
def configure_dataset(root,config_path=None):
    ...
    global KEY_COUNT,VALUE_BASE,VOCAB
    path=config_path or root/'dataset_config.json'
    if not path.exists():return
    config=json.loads(path.read_text())
    KEY_COUNT=int(config.get('key_count',128))
    VALUE_BASE=KEY_BASE+KEY_COUNT;VOCAB=VALUE_BASE+VALUES
    ...
    assert 1<=KEY_COUNT and VOCAB<65536
...
def apply_model_options(model,recipe):
    """Apply recorded initialization and existing memory options reproducibly."""
    tau=recipe.get('memory_timescale_initialization',0)
    with torch.no_grad():
        for module in model.modules():
            if tau and module.__class__.__name__=='GatedDeltaNet':
                delta=1/(tau*module.A_log.float().exp())
                module.dt_bias.copy_(torch.log(torch.expm1(delta)))
            if recipe.get('gdn_memory_scalar_decay') is False and hasattr(module,'transport_scalar_decay'):
                module.transport_scalar_decay=False
...
def run_one(args,condition,family,d,seed,manifest):
    ...
    recipe=dict(condition=condition,family=family,d=d,width=args.width,seed=seed,
        ...
        example_presentations=args.epochs*manifest['train_examples'],lr=args.lr,weight_decay=args.weight_decay,
    ...
    if args.gdn_memory_no_scalar_decay:
        if family!='gdn_current' or d<2:raise ValueError('No-scalar-decay ablation requires SMat GDN')
        recipe['gdn_memory_scalar_decay']=False
    ...
    apply_model_options(model,recipe)
...
def main():
    ...
    p.add_argument('--gdn-memory-no-scalar-decay',action='store_true')
