"""Active d=2/3 PG19 arms; d=4 launches are disabled at user request."""
import hashlib
import json
import os
...
def run(family:str,d:int,pilot:bool=True,fused_write:bool=True,batch:int=0,activation_checkpointing:bool=False):
    if d == 4:
        raise ValueError('PG19 d=4 runs are disabled at user request (2026-09-17)')
    assert family in ('gdn','mamba2') and d in (2,3)
    batch=batch or (2 if family=='gdn' or d==2 else 1)
...
def sweep(pilot:bool=True,fused_write:bool=True,batch:int=0,activation_checkpointing:bool=False):
    calls={f'{f}-d{d}':run.spawn(f,d,pilot,fused_write,batch,activation_checkpointing) for f in ('gdn','mamba2') for d in (2,3)}
