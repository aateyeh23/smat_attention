...
app=modal.App('pg19-tail-memory-probe')
...
if modal.is_local():
    ...
    image=image.add_local_file(LOCAL/'lm/probe_pg19_tail.py','/opt/pg19-rank64/lm/probe_pg19_tail.py')
...
def probe(arm:str):
    ...
    out=Path('/results/tail-2k')/arm;out.mkdir(parents=True,exist_ok=True)
    with (out/'probe.log').open('w') as log:
        child=subprocess.Popen([sys.executable,'-P','-u','/opt/pg19-rank64/lm/probe_pg19_tail.py',
