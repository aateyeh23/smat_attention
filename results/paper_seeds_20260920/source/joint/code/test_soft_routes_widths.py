...
for width,heads in ((16,1),(32,2),(64,2)):
 ...
 m=SmatGDNReset(width,d=3,memory_update='delta',memory_key_mode='tied_causal',memory_route_soft=True,anneal_steps=1).cuda()
 ...
 for length in (64,128,256):
  ...
  ca=next(iter(m.ca[str(length)].values()))
  assert ca.anneal==0.0 and ca._sparse[0].shape[-1]>1
  with torch.no_grad():
   before=m(u);m.eval();after=m(u);m.train()
   assert ca.anneal==0.0
