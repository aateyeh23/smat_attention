...
if modal.is_local():
    ...
    image = (cached_image
             .add_local_file(LOCAL / 'lm/gdn_smat_scale.py', '/opt/smat/lm/gdn_smat_scale.py')
...
def pilot(d: int, fast: bool = False):
    ...
    command = [sys.executable, '-u', '/opt/smat/lm/gdn_smat_scale.py',
    ...
    with log.open('w') as fp:
        child = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                                 text=True, cwd='/opt/smat/the GPU cluster')
