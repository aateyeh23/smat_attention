...
VARIANTS={
    ...
    '31_w64_lr003_d2':dict(name='decay_no_rescale_lr003',d=2,lr=.003,kwargs=dict(
        write_hash_neighbor_grad=True,transport_scalar_decay=True,
        detach_write_hash_input=True,memory_plant=False,
        memory_incidence_rescale=False)),
    '32_w64_lr003_d3':dict(name='decay_no_rescale_lr003',d=3,lr=.003,kwargs=dict(
        write_hash_neighbor_grad=True,transport_scalar_decay=True,
        detach_write_hash_input=True,memory_plant=False,
        memory_incidence_rescale=False)),
}
...
def run(trial: str='23_decay_no_rescale', continue_below_baseline: bool=False,
    ...
    source_paths.update({name:'/opt/smat/'+name+'.py' for name in (
    ...
    def execute(command,name):
        with (root/name).open('a') as log:
            child=subprocess.Popen(command,cwd='/opt/smat/the GPU cluster',env=env,
