...
class SmatMamba2MR(nn.Module):
    def __init__(self, d_model, layer_idx=0, d=2, d_state=128, headdim=None, mask_bank=True, n_P=0,
        ...
        super().__init__()
        # read == "chash": user's content-addressed assignment (content_addr.ContentAssign): profile = hash(key-side
        ...
        if g_write_mode not in ("shared", "independent"):
        ...
        if g_write_mode == "independent" and not write_gate:
            raise ValueError("independent G write gate requires write_gate=True")
        self.g_write_mode = g_write_mode
        self.g_write_proj = None
        self.track_g_write_stats = False
        if d >= 2 and g_write_mode == "independent":
            ...
            nn.init.constant_(self.g_write_proj.bias, math.log(g_write_init / (1 - g_write_init)))

    def _g_write_weights(self, u, dts):
        """G's per-token/head write strength; independent mode replaces dt, not multiplies it.

        This leaves the recurrence and G's optional decay untouched.
        """
        if self.g_write_proj is not None:
            return torch.sigmoid(self.g_write_proj(u.float()))
