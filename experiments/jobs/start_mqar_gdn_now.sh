...
pids=()
for width in ${MQAR_WIDTHS:-16 32}; do
    env ZOO_DM="$width" python -u -m zoology.launch experiments/zoology/gdn_reset.py >> "$MQAR_RUN_DIR/w${width}-d3.log" 2>&1 &
    pids+=("$!")
done
status=0
