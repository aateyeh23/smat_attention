...
export MQAR_RUN_DIR="$PWD/results/mqar_mamba_reset/w16_hd16_ds16_independent_s123"
export TRITON_CACHE_DIR="$PWD/results/mqar_gdn_reset/check_cache"
mkdir -p "$MQAR_RUN_DIR"
cp experiments/zoology/mamba_reset.py zoo_mqar_resume.py zoo_smat_mixer.py run_mqar_mamba_w16.sh "$MQAR_RUN_DIR/"
python -u test_mqar_mamba_reset.py >> "$MQAR_RUN_DIR/test.log" 2>&1
pids=()
for d in ${M2_DS:-1 2 3 4}; do
    env ZOO_DS="$d" python -u -m zoology.launch experiments/zoology/mamba_reset.py >> "$MQAR_RUN_DIR/w16-d${d}.log" 2>&1 &
    pids+=("$!")
done
status=0
