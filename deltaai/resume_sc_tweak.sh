...
OLD=$1; cd /u/archerdw/smat_attention/deltaai
while squeue -h -j $OLD -o "%T" 2>/dev/null | grep -q RUNNING; do sleep 30; done
for i in $(seq 1 240); do NEW=$(squeue -u archerdw -h -n pg19 -t RUNNING -o "%i" | head -1); [ -n "$NEW" ] && break; sleep 30; done
...
ENVS="PYTHONPATH=/u/archerdw/smat_attention/deltaai:/u/archerdw/.local/lib/python3.12/site-packages:/u/archerdw/zoology PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True"
launch() { name=$1; extra=$2; grep -q "FINAL\|SOLVED" logs/sc4k-tweak-$name.out 2>/dev/null && return; E2=""; case "$extra" in *"--arm gdn"*) E2="PYTHONPATH=/u/archerdw/triton371:/u/archerdw/smat_attention/deltaai:/u/archerdw/.local/lib/python3.12/site-packages:/u/archerdw/zoology FLA_TILELANG=0";; esac; nohup srun --jobid=$NEW --overlap --network=no_vni -N1 -n1 --export=ALL bash -c "module load python/miniforge3_pytorch/2.11.0 >/dev/null 2>&1; cd /u/archerdw/smat_attention/deltaai; env $ENVS $E2 python -u lm/sc_train.py $BASE $extra --ckpt /work/hdd/bekw/archerdw/ckpt/sc4k-tweak-$name.pt 2>&1 | grep --line-buffered -v -i 'warn\|hi mamba\|custom_\|torchao'" < /dev/null >> logs/sc4k-tweak-$name.out 2>&1 & sleep 2; }
