...
def main():
    ...
    for config in CONFIGS:
        ...
        original=Path('/u/archerdw/smat_attention/deltaai/results/learned_mkar_original_mamba_transport_normalized_20260923/source/experiment/learned_mixers.py')
        spec=importlib.util.spec_from_file_location('original_mamba_factory',original)
        mod=importlib.util.module_from_spec(spec);spec.loader.exec_module(mod)
        with patch('campaign_model.make_mixer',mod.make_mixer):reference=Model(config,123).cuda()
