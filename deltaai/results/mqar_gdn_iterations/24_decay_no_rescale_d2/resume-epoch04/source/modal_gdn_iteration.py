...
def run(trial: str='23_decay_no_rescale', continue_below_baseline: bool=False):
    ...
    manifest=dict(trial=trial,**variant,width=32,heads=2,head_dim=16,
    ...
    env=dict(os.environ,PYTHONPATH='/opt/iterate:/opt/transport:'+os.environ['PYTHONPATH'],
             GDN_VARIANT_JSON=json.dumps(variant),GDN_BASELINE_HISTORY='/opt/iterate/gdn-baseline.jsonl',
             MQAR_RUN_DIR=str(root),MQAR_MAX_MINUTES='165',ZOO_DM='32',ZOO_DS=str(d),
    ...
    def execute(command,name):
        with (root/name).open('a') as log:
            ...
            for line in child.stdout:
                ...
                if any(marker in line for marker in ('CHECKPOINT','GATE ','PASS ','GRADIENT_CLIP ','OPTIMIZER_GROUPS ','SOURCE_AUDIT ','RESUMED ','TRAINING ')):
