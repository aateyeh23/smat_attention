...
class _DeltaWriteHashGradient(torch.autograd.Function):
    @staticmethod
    def forward(ctx, memory, keys, values, indices, weights, beta, cells):
        ctx.save_for_backward(keys, values, indices, weights.detach(), beta)
        ctx.cells = cells
        return memory
    ...
    def backward(ctx, memory_grad):
        ...
        keys, values, indices, weights, beta = ctx.saved_tensors
        # Recompute neighboring histories only in backward. At the hard weights
        # these have the same nonzero updates as the single-route forward pool.
        # Payload/beta gradients come exclusively from that original hard pool.
        with torch.enable_grad():
            route_weights = weights.detach().requires_grad_(True)
            shadow = pool_delta(keys, values, indices, route_weights, beta, ctx.cells)
            route_grad, = torch.autograd.grad(shadow, route_weights, memory_grad)
        return memory_grad, None, None, None, route_grad, None, None


def with_delta_write_hash_gradient(memory, keys, values, indices, weights, beta, cells):
    """One physical delta update forward; full hard-STE route gradient backward."""
    return _DeltaWriteHashGradient.apply(
        memory, keys.detach(), values.detach(), indices, weights, beta.detach(), cells)
