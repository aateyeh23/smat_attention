...
class Trainer:
    ...
    def fit(self):
        ...
        # Modules that a mixer builds lazily at its first forward (e.g. the per-length content-hash modules of the
        # SMAT mixers) must exist BEFORE the optimizer collects model.parameters(), or they never train.  Run one
        # no-grad forward on a training batch first.  (Before 2026-09-12 the content hash was a frozen random
        # projection in every Zoology run.)
        n_before = sum(1 for _ in self.model.parameters())
        with torch.no_grad():
            batch = next(iter(self.train_dataloader))
            inputs = batch[0] if isinstance(batch, (tuple, list)) else batch
            self.model(inputs.to(self.device))
        n_after = sum(1 for _ in self.model.parameters())
        print(f"[zoology] parameter tensors before/after prebuild forward: {n_before}/{n_after}", flush=True)
