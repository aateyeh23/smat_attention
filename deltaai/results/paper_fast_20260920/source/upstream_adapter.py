...
_UPSTREAM = Path(__file__).resolve().parent/'upstream/hattention'
...
def log_linear(q,k,v,g,lam,beta=None):
    ...
    # Mamba's dt multiplication promotes v to FP32 outside the operator.
    # Upstream dot kernels require matching q/k/v dtypes.
    v = v.to(q.dtype)
    padding = (-length) % 64
    lam=lam[...,:((length+padding-1).bit_length()+1)]
    q,k,v = [F.pad(x, (0, 16-x.shape[-1], 0, 0, 0, padding)) for x in (q,k,v)]
    ...
    return output[:,:length,:,:value_dim].to(output_dtype).contiguous()
