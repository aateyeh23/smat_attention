...
def main():
    ...
    out=dict(passed=True,completed_runs=len(audited)//10,planned_runs=24,checks=audited)
    (ROOT/'result_audit.json').write_text(json.dumps(out,indent=2)+'\n')
    print(f'Recomputed saved predictions for {out["completed_runs"]}/24 runs; controls saved.')
