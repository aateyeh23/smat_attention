...
def length(c,k): return max(64,4*math.ceil((2+2*c+4*c*k)/4))
def sha_file(path):
    h=hashlib.sha256()
...
def prepare(root):
    ...
    manifest=dict(version=2,data_seed=DATA_SEED,vocab=VOCAB,values=VALUES,
        train_examples=180000,validation_examples=5000,test_examples=10000,
        encoding='BOS context-blocks SEP permuted inquiry-blocks; masked answer slots',
    ...
    for c,k,train_n in CELLS:
        ...
        for split,code,n in [('train',1001,train_n),('validation',2001,1000),('test',3001,2000)]:
...
def run_one(args,condition,family,d,seed,manifest):
    ...
    recipe=dict(condition=condition,family=family,d=d,width=args.width,seed=seed,
        ...
        total_steps=args.epochs*steps_per_epoch,examples_per_epoch=180000,
        example_presentations=args.epochs*180000,lr=.01,weight_decay=.1,
        ...
        lengths=list(dict.fromkeys(length(c,k) for c,k,_ in CELLS)),validation_examples=5000,test_examples=10000,
    ...
    result=dict(base,complete=True,epochs=epoch,steps=steps,examples_seen=epoch*180000,
