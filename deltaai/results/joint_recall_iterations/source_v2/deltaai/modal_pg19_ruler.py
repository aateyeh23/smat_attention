...
app = modal.App('pg19-ruler-mk-niah1')
...
if modal.is_local():
    ...
    image = image.add_local_file(LOCAL/'lm/eval_pg19_ruler.py', '/opt/pg19-rank64/lm/eval_pg19_ruler.py')
...
def evaluate(arm: str, limit: int=0, batch: int=4):
    ...
    output = Path('/results/mk-niah1-16k-500m')/('full' if not limit else f'pilot-{limit}')/arm
    ...
    try:
        while True:
            ...
            command = [sys.executable, '-P', '-u', '/opt/pg19-rank64/lm/eval_pg19_ruler.py',
