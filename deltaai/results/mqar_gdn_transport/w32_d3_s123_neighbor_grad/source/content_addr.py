...
class ContentAssign(nn.Module):
    ...
    def _topk_reads(self, u):
        ...
        if self.read_backend == "triton" and self.read_k == 4:
            ...
            idx, weights = top4_softmax(logits, self.b_read)
        else:
            logits = logits + self.b_read[None, :, None, :]
    ...
    def forward(self, u, Phi, Psi, Vb, n, emb=None, key_src=None, key_w=None):
        ...
        if neighbor_grad and (self.read_k is None or not getattr(self, "sparse_ops", False)
                              or self.anneal != 1.0 or getattr(self, "delta_updates", False)):
            raise ValueError("Neighbor write gradients require hard additive pooling and top-k reads")
        Wk = self._cells(hk[:, :n], plant_at=True, tok_w=key_w,
