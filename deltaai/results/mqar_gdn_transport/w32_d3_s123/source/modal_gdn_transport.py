...
if modal.is_local():
    ...
    image=cached_image
    for filename in ('smat_gdn_transport.py','zoo_gdn_transport.py','zoo_gdn_transport_configs.py','test_gdn_transport_gpu.py'):
...
def execute(command,path):
    env=dict(os.environ,PYTHONPATH='/opt/transport:'+os.environ['PYTHONPATH'],
             MQAR_RUN_DIR='/transport-results/seed123',MQAR_MAX_MINUTES='165',
...
def run():
    volume.reload()
    root=Path('/transport-results/seed123');root.mkdir(exist_ok=True,parents=True)
    ...
    volume.commit()
    print('GPU_VALIDATED: starting one 32-epoch width-32 d=3 run, lr=0.01 seed=123',flush=True)
    execute([sys.executable,'-u','-m','zoology.launch','/opt/transport/zoo_gdn_transport_configs.py'],root/'train.log')
