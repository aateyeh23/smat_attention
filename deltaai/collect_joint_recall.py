...
planned_count=len(json.loads((ROOT/'planned_runs.json').read_text()))
...
report=['# Block-context joint recall: full epoch budget','',
        f'Completed {sum(p["complete"] for p in progress)} of {planned_count} planned runs. '
...
findings=['# Joint recall findings','',f'Completed {finished}/{planned_count} planned runs.','']
if finished<planned_count:
...
print(f'{sum(p["complete"] for p in progress)}/{planned_count} complete; {len(progress)} runs started')
