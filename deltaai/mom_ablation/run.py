...
def main():
    ...
    with (folder/'run.lock').open('a') as lock:
        ...
        if Path(task['result']).exists():
            return
        import joint_recall
        from mom_model import apply_ablation
        original = joint_recall.make_model

        def make_model(*a, **kw):
            model = original(*a, **kw)
            audit = apply_ablation(model, task['variant'], 0)
            audit.update(training_seed=task['seed'], source_manifest_sha256=digest,
                         initialization='fresh MoM mixers; embedding, residual norms and head preserved from paired SMAT initialization')
