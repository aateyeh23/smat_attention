...
export WANDB_MODE=disabled ZOO_EPOCHS=32 ZOO_MR3=1 ZOO_HEADDIM=16 ZOO_DSTATE=16 ZOO_LAMACT=sigmoid ZOO_POOL=delta ZOO_READ=content ZOO_DS=3 ZOO_LRS=1e-2
...
for dm in 16 32 64; do
  ( ZOO_DM=$dm python -m zoology.launch zoo_smat_configs.py 2>&1 | eval "$FILT" > logs/zoo-ds16-cread-d3-dm$dm.out 2>&1 ) &
done
wait
