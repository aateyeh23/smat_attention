...
def check(d):
    if d == 2:
        from test_write_hash_fused_gpu import check as check_write_gradient
        check_write_gradient()
    ...
    if d == 2:
        assert ca.mode == 'point' and ca.dim == 1
        assert ca.read_k == 4 and ca.W_read.shape[1] == ca.N0
