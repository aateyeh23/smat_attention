...
def main(args):
    ...
    provenance = dict(arm=checkpoint.parent.name, checkpoint=str(checkpoint),
        ...
        evaluator_sha256=digest(__file__), decoder_sha256=digest(Path(__file__).with_name('pg19_recurrent.py')), decoding='cached_recurrent_greedy', eos_token_id=tokenizer.eos_token_id,
    ...
    while len(predictions) < len(rows):
        ...
        try:
            with torch.inference_mode(), torch.autocast('cuda', dtype=torch.bfloat16):
                decoder = RecurrentDecoder(model)
                batch_started = time.monotonic()
                logits = decoder.prefill([row['input_ids'] for row in batch])
                torch.cuda.synchronize()
                prefill_seconds = time.monotonic()-batch_started
                ...
                torch.cuda.synchronize()
                print('CACHED_TIMING '+json.dumps(dict(batch=len(batch), prefill_seconds=prefill_seconds,
                    decode_seconds=time.monotonic()-decode_started, decode_steps=step)), flush=True)
                del decoder, logits
