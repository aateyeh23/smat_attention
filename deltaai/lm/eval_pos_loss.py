...
import argparse, sys, json, math, numpy as np, torch, torch.nn.functional as F
...
ap.add_argument("--data", default="/work/hdd/bekw/archerdw/pg19"); ap.add_argument("--n_tokens", type=float, default=3.9e7)
...
spec = importlib.util.spec_from_file_location("tl", "/u/archerdw/smat_attention/deltaai/lm/train_lm2.py")
# build the model exactly as trained without running the training loop
src = open("/u/archerdw/smat_attention/deltaai/lm/train_lm2.py").read().split("model = LM().to(dev)")[0]
...
z = ns["z"]
T = a.seq_len; data = ns["load_split"](a.split)
n_seq = int(a.n_tokens // T); n_seq = min(n_seq, (len(data) - 1) // T)
...
with torch.no_grad():
    for s in range(0, n_seq, a.batch):
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16): lg = model(xb[:, :-1])
        nll = F.cross_entropy(lg.float().transpose(1, 2), xb[:, 1:], reduction="none")   # (b, T)
