"""Continue frozen 500M PG19 training states at a constant learning rate."""
import argparse
...
elif os.environ.get('PG19_MEMORY_VARIANT') in ('gdn_baseline', 'mamba2_baseline'):
...
def train(args):
    variant_args = {}
    if os.environ.get('PG19_MEMORY_VARIANT') in ('gdn_baseline', 'mamba2_baseline'):
        variant_args['family'] = os.environ['PG19_MEMORY_VARIANT'].removesuffix('_baseline')
    ...
    recipe = dict(config=asdict(cfg), target_tokens=args.tokens, target_rows=target_rows,
                  ...
                  warmup_tokens=args.warmup_tokens, lr_schedule='constant',
                  resume_from=str(Path(args.resume_from)), train_sha256=manifest['splits']['train']['sha256'],
    ...
    resume_path = checkpoint if checkpoint.exists() else Path(args.resume_from)
    if not resume_path.exists():
        raise FileNotFoundError(resume_path)
    if resume_path.exists():
        saved = torch.load(resume_path, map_location='cpu', weights_only=False)
        parent = saved['recipe']
        for key in ('config', 'batch', 'accumulation', 'global_batch_rows',
                    'train_sha256', 'eval_rows', 'seed'):
            if parent[key] != recipe[key]:
                raise ValueError(f'Continuation changed {key}')
        if resume_path == checkpoint:
            if parent != recipe:
                raise ValueError('Changed continuation recipe')
        elif saved['cursor']*cfg.length != 500006912 or not saved['complete']:
            raise ValueError('Expected completed 500M resumable checkpoint')
        model.load_state_dict(saved['model'])
        optimizer.load_state_dict(saved['optimizer'])
        cursor, step = saved['cursor'], saved['step']
        elapsed_before = saved['elapsed_seconds'] if resume_path == checkpoint else 0.0
...
if __name__ == '__main__':
    ...
    p.add_argument('--tokens', type=int, default=750_000_000)
    ...
    args = p.parse_args()
    if args.lr <= 0 or args.warmup_tokens != 0:
        raise ValueError('Continuation requires positive constant LR and no warmup')
    if os.environ.get('PG19_OPTIMIZED') == '1':
