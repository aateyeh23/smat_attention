...
class SmatGDNTransport(SmatGDNReset):
    def __init__(self, *args, write_hash_neighbor_grad=False,
                 ...
                 joint_hash_balance=False, transport_enabled=True,
                 periodic_hash=False, profile_delta=False,
                 memory_concurrent_reads=False, **kwargs):
        ...
        self.periodic_hash = bool(periodic_hash)
        self.profile_delta = bool(profile_delta)
        self.memory_concurrent_reads = bool(memory_concurrent_reads)
        if self.memory_concurrent_reads and not self.memory_read_address:
            raise ValueError('Concurrent reads require shared address scoring')
        if self.profile_delta and self.transport_enabled:
            raise ValueError('Profile delta uses normalized, untransported keys')
        ...
        for modules in self.ca.values():
            for ca in modules.values():
                ...
                ca.periodic_hash = self.periodic_hash
                ca.concurrent_reads = self.memory_concurrent_reads
                if ca.concurrent_reads and (ca.mode != 'plane' or ca.D < 4):
                    raise ValueError('Concurrent four-read geometry needs at least four directions')
        ...
        self.memory_update = 'profile_delta' if self.profile_delta else 'transport_additive'
        ...
        if not self.disable_g:
            with torch.autocast('cuda', enabled=False):
                ...
                if self.transport_enabled:
                    ...
                    far_k = kn[:,:n] if self.profile_delta else kn[:,:n] * beta[:,:n,...,None]
                    recent_q = qn[:,n:]
                ...
                ca.delta_updates = self.profile_delta
                ca.detach_balance_weights = self.detach_write_hash_input
                ...
                hash_write_weight = (beta[:,:n].detach()
                    if self.detach_write_hash_input and not self.profile_delta else beta[:,:n])
