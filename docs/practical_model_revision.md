...
- `zoo_gdn_transport.py`: transport-additive default, local resets,
  read gate, r^(-1/2) query scaling and output processing.
- `smat_gdn_transport.py`: ordered transition products and shifted
  reverse prefix scan, with beta included exactly once.
- `zoo_mamba_four_reads.py`: recall wrapper, independent write gate,
  memory decay disabled.
- `lm/mamba_smat_scale.py`: PG-19 wrapper, memory decay enabled.
- `zoo_smat_mixer.py`: Mamba local recurrence, memory weighting,
  scalar decay factoring and gated RMSNorm output.
- `lm/pg19_recurrent.py`: cached learned top4 selection, recurrent GDN
