...
- Smoke run 3078180 (GPU, torch 2.11.0+cu130, triton 3.5.1): all tests
...
(expand_v 2 at one head faults intermittently in fla 0.5.2's chunk kernel on GPU -- 9/12 launches -- so expand_v 1 everywhere.)
...
### Content-hashed SMAT (user's `content_addr.ContentAssign`) in the Zoology pipeline, 2026-09-11 (job 3132062, `run_zoo_chash.sbatch`)
...
causal conv (width 4, init uniform over lags) on the key-side hash input instead of user's fixed shift of 1.
...
### Content-hashed SMAT (user's `content_addr.ContentAssign`) in the Zoology pipeline, width 16, hd16 / ds16, seed 123, 2026-09-11 (job 3132062, `run_zoo_chash.sbatch`)
...
`hash_src=embed, freeze` = user's frozen recipe: a fixed random projection of the token EMBEDDING (context-free),
...
5M test bytes (1220 sequences), window 501.  Fixes needed on the way: user's point read / pooling materialised
...
Hard-phase (anneal done) layer cost at width 512 / T 16K / batch 4, single GPU, point read:
...
path: 187 ktok/s vs 59 ktok/s for the earlier dense point read (3.2x) on one GPU.
