...
- `zoo_smat_mixer.py`: shared B/C memory features, independent/residual
  memory writes, optional scalar memory decay, and additive readout.
- `zoo_mamba_reset_configs.py`: checked Mamba reset configuration.
- `zoo_smat_gdn.py`: reset GDN, causal memory features, soft routing,
  reused beta gates, and final branch combination.
- `content_addr.py`: sparse write/read assignments and incidence readout.
- `smat_delta_pool.py`: per-profile delta updates with beta times the
