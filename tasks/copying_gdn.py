...
ap.add_argument("--g_write_mode", choices=["shared", "independent", "residual"], default="shared",
                help="G writes: dt (shared), separate sigmoid (independent), or dt times a sigmoid correction initialized to 1 (residual)")
ap.add_argument("--g_write_init", type=float, default=0.1, help="initial independent G write probability")
ap.add_argument("--g_write_lr_scale", type=float, default=1.0, help="learning-rate multiplier for G write projection only")
ap.add_argument("--memory_update", choices=["additive", "delta"], default="delta")
args = ap.parse_args()
if not math.isfinite(args.g_write_lr_scale) or args.g_write_lr_scale <= 0:
    ap.error("--g_write_lr_scale must be finite and positive")
...
class Block(nn.Module):
    def __init__(self, i):
        ...
        if args.arm == "gdn_smat":
            from zoo_smat_gdn import SmatGDNReset
            hd = args.gdn_headdim or args.headdim
            self.mixer = SmatGDNReset(args.d_model, layer_idx=i, d=args.d,
                headdim=hd, n_heads=max(1,args.d_model//hd), expand_v=2,
                reset=bool(args.reset), anneal_steps=args.anneal, balance_coef=args.balance,
                hash_codim=args.hash_codim, prebuild_lengths=(args.seq_len,),
                memory_update=args.memory_update,
                memory_hash_source='gdn' if args.hash_src == 'ssm' else 'hidden')
            return
        if args.arm == "gdn":
...
for layer, block in enumerate(model.blocks):
    spec = getattr(block.mixer, "specs", {}).get(args.seq_len)
    if spec is not None:
        print(f"G geometry layer={layer} effective_d={spec.dim + 1} q={spec.q} cells={spec.N0}", flush=True)
decay, no_decay = [], []
...
if args.g_write_lr_scale != 1.0:
    from sc_optim import write_gate_groups
    groups = write_gate_groups(model, args.wd, args.g_write_lr_scale)
opt = torch.optim.AdamW(groups, lr=args.lr, betas=(0.9, 0.95))
print(f"optimizer G-write LR scale={args.g_write_lr_scale}; groups="
      f"{[(len(g['params']), g.get('lr_scale', 1.0)) for g in opt.param_groups]}", flush=True)
...
if os.path.exists(args.ckpt):
    ...
    if ck["args"].get("g_write_lr_scale", 1.0) != args.g_write_lr_scale:
        raise ValueError("checkpoint G write LR scale does not match this run; use a new checkpoint path")
    if ck["args"].get("arm") != args.arm or ck["args"].get("memory_update", "delta") != args.memory_update:
    ...
    if "rng_cpu" in ck:
        ...
        torch.cuda.set_rng_state_all([state.cpu() for state in ck["rng_cuda"]])
...
def save_checkpoint():
    torch.save({"model": model.state_dict(), "opt": opt.state_dict(), "step": step,
                ...
                "rng_cuda": torch.cuda.get_rng_state_all()}, args.ckpt)
    status = dict(step=step, complete=bool(step >= args.steps or hits >= 2), args=vars(args))
    with open(os.path.splitext(args.ckpt)[0]+".json", "w") as f: json.dump(status,f,indent=2)


t0 = time.time(); losses = []; hits = 0
while step < args.steps:
    xb, yb = make_batch(args.batch); xb, yb = xb.to(dev, non_blocking=True), yb.to(dev, non_blocking=True)
    for g in opt.param_groups: g["lr"] = lr_at(step) * g.get("lr_scale", 1.0)
